...
function generateRandomSeries() {
    ...
    const series = letters[Math.floor(Math.random() * letters.length)] + 
...
document.querySelector(".generate-btn").addEventListener('click', () => {
    ...
    const randomFutureDate = new Date(today);
    randomFutureDate.setFullYear(today.getFullYear() + 10);
    randomFutureDate.setMonth(Math.floor(Math.random() * 12));
    randomFutureDate.setDate(Math.floor(Math.random() * 28) + 1);

    const issueDate = threeYearsAgo.toLocaleDateString('pl-PL');
    const expiryDate = randomFutureDate.toLocaleDateString('pl-PL');

    // Store dates in localStorage for card.html
    ...
    document.querySelectorAll(".input_holder").forEach((element) => {
        ...
        switch(input.id) {
            ...
            case "city":
                ...
                break;
            case "seria_numer":
                randomValue = generateRandomSeries();
                break;
            case "fatherName":
                randomValue = getRandomElement(["Adam", "Jan", "Piotr", "Marek", "Tomasz", "Andrzej", "Paweł", "Michał"]);
                break;
            case "motherName":
                randomValue = getRandomElement(["Anna", "Maria", "Katarzyna", "Małgorzata", "Barbara", "Ewa", "Krystyna"]);
                break;
        }
